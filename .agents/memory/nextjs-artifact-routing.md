...
- Never run `next build` while the dev server is up — they share `.next/` and it corrupts the dev bundle (module-not-found/webpack `call`/routes-manifest ENOENT errors). Fix: `rm -rf artifacts/nextjs-app/.next` + restart the web workflow. This applies to review/verification subagents too — tell them explicitly not to run `next build`. (Publishing is safe: deployment builds run in a separate environment.)
